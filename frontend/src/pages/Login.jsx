...
export default function Login() {
  ...
  try {
    ...
    if (requierePin) payload.pin = form.pin
    ...
    await login(form.username, form.password, form.pin)
    navigate('/dashboard')
  } catch {
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {!requierePin ? (
            ...
            <div>
              <div className="bg-blue-50 rounded-xl p-4 mb-4 text-center">
                <p className="text-blue-700 font-semibold text-sm">🔐 Segundo factor de autenticación</p>
                <p className="text-blue-500 text-xs mt-1">Ingrese su PIN de 6 dígitos</p>
              </div>
              ...
              <input type="password" value={form.pin} maxLength={6}
                ...
                className="w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm text-center text-2xl tracking-widest"
                placeholder="● ● ● ● ● ●" required/>
              <button type="button" onClick={() => { setRequierePin(false); setForm({...form, pin: ''}) }}
          ...
          <button type="submit" disabled={cargando}
            ...
            {cargando ? 'Verificando...' : requierePin ? 'Confirmar PIN' : 'Ingresar al Sistema'}
          </button>
        </form>
